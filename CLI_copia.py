...
# Incio logging- jlgr
log_filename = datetime.now().strftime("LOG_%d%m%Y.txt")
logging.basicConfig(filename=log_filename, level=logging.INFO, format='%(asctime)s - %(username)s - %(role)s - %(message)s')

def log_event(username, role, message):
    extra = {'username': username if username else 'Unknown', 'role': role if role else 'None'}
...
def handle_choice(choice, username, role):
    try:
        ...
        func = menu_functions.get(choice, lambda: console.print("[red]Opción no válida.[/]"))
        log_event(username, role, f"Función seleccionada para la opción {choice}")
        return func()
    except Exception as e:
